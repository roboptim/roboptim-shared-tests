...
namespace roboptim
{
  namespace schittkowski
  {
	...
	static const double fx;
      ...
      const double ExpectedResult::f0 = 13.;
      const double ExpectedResult::x[] = {.5, -.5, .5};
	...
	grad.insert (1) = 2. * x[0] + 4. * x[1] + 2 * x[2];
	grad.insert (2) = 2 * x[1] + 2. + x[2];
      }
...
BOOST_AUTO_TEST_CASE (schittkowski_problem28)
{
  ...
  OptimizationLogger<solver_t> logger
    (solver,
     "/tmp/roboptim-shared-tests/" SOLVER_NAME "/schittkowski/problem-28");
