...
namespace roboptim
{
  namespace schittkowski
  {
	...
	grad.coeffRef (3) = -1.;
      ...
      template <typename T>
      void
      G4<T>::impl_gradient (gradient_ref grad, const_argument_ref, size_type)
	const
      {
	grad[0] = 0.;
	grad[1] = 0.;
	...
	grad.coeffRef (3) = -2.;
      ...
      template <typename T>
      void
      G5<T>::impl_gradient (gradient_ref grad, const_argument_ref, size_type)
	const
      {
	grad[0] = 0.;
	grad[1] = 0.;
	...
	: GenericDifferentiableFunction<T>
      ...
      template <typename T>
      void
      G6<T>::impl_compute (result_ref result, const_argument_ref x)
	const
      {
	result[0] = 5. - x[2] - x[3];
      }
	...
	grad.coeffRef (3) = -1.;
      ...
      template <typename T>
      void
      G6<T>::impl_gradient (gradient_ref grad, const_argument_ref, size_type)
	const
      {
	grad[0] = 0.;
	grad[1] = 0.;
...
BOOST_AUTO_TEST_CASE (schittkowski_problem44)
{
  ...
  SET_OPTIMIZATION_LOGGER (solver, "schittkowski/problem-44");
