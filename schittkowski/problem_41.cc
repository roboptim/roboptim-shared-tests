...
namespace roboptim
{
  namespace schittkowski
  {
	...
	static const double fx;
      ...
      const double ExpectedResult::x[] = {2./3., 1./3., 1./3., 2.};
      const double ExpectedResult::fx = 52./27.;
	...
	result[0] = x[0] + 2*x[1] + 2*x[2] - x[3];
      ...
      (gradient_ref grad, const_argument_ref&, size_type) const throw ()
      {
	grad.insert (0) = 1;
	grad.insert (1) = 2;
	grad.insert (2) = 2;
...
BOOST_AUTO_TEST_CASE (schittkowski_problem41)
{
  ...
  OptimizationLogger<solver_t> logger
    (solver,
     "/tmp/roboptim-shared-tests/" SOLVER_NAME "/schittkowski/problem-41");
