...
namespace roboptim
{
  namespace schittkowski
  {
	...
	static const double fx;
      ...
      const double ExpectedResult::f0 = 10.;
      const double ExpectedResult::x[] = {1., 1.};
      const double ExpectedResult::fx = 2.;
	...
	: GenericDifferentiableFunction<T>
      ...
      template <typename T>
      void
      G4<T>::impl_compute (result_ref result, const_argument_ref& x)
	const throw ()
      {
	...
	grad.insert (1) = -1.;
      ...
      template <typename T>
      void
      G4<T>::impl_gradient (gradient_ref grad, const_argument_ref& x, size_type)
	const throw ()
      {
	grad[0] = 2. * x[0];
	grad[1] = -1.;
	...
	: GenericDifferentiableFunction<T>
      ...
      template <typename T>
      void
      G5<T>::impl_compute (result_ref result, const_argument_ref& x)
	const throw ()
      {
	result[0] = x[1] * x[1] - x[0];
      }
	...
	grad.insert (1) = 2. * x[1];
      ...
      template <typename T>
      void
      G5<T>::impl_gradient (gradient_ref grad, const_argument_ref& x, size_type)
	const throw ()
      {
	grad[0] = -1.;
	grad[1] = 2. * x[1];
    ...
    } // end of namespace problem23.
  } // end of namespace schittkowski.
} // end of namespace roboptim.

BOOST_FIXTURE_TEST_SUITE (schittkowski, TestSuiteConfiguration)

BOOST_AUTO_TEST_CASE (schittkowski_problem23)
{
  using namespace roboptim;
  using namespace roboptim::schittkowski::problem23;
  ...
  problem.argumentBounds ()[0] = F<functionType_t>::makeInterval (-50., 50.);
  problem.argumentBounds ()[1] = F<functionType_t>::makeInterval (-50., 50.);
  ...
  problem.addConstraint (g4, G<functionType_t>::makeLowerInterval (0.));
  ...
  OptimizationLogger<solver_t> logger
    (solver,
     "/tmp/roboptim-shared-tests/" SOLVER_NAME "/schittkowski/problem-23");
