...
namespace roboptim
{
  namespace schittkowski
  {
	...
	grad.insert (1) = 2. * x[1];
      ...
      template <typename T>
      void
      G3<T>::impl_gradient (gradient_t& grad, const argument_t& x, size_type)
	const throw ()
      {
	grad[0] = 9. * 2. * x[0];
	grad[1] = 2. * x[1];
	...
	grad.insert (1) = -1.;
      ...
      template <typename T>
      void
      G4<T>::impl_gradient (gradient_t& grad, const argument_t& x, size_type)
	const throw ()
      {
	grad[0] = 2. * x[0];
	grad[1] = -1.;
	...
	grad.insert (1) = 2. * x[1];
      ...
      template <typename T>
      void
      G5<T>::impl_gradient (gradient_t& grad, const argument_t& x, size_type)
	const throw ()
      {
	grad[0] = -1.;
	grad[1] = 2. * x[1];
