...
namespace roboptim
{
  namespace schittkowski
  {
	...
	const
      {
	result[0] = std::pow (x[0], 3) + std::pow (x[1], 2) - 1;
	result[1] = std::pow (x[0], 2) * x[3] - x[2];
	result[2] = std::pow (x[3], 2) - x[1];
      }
...
BOOST_AUTO_TEST_CASE (schittkowski_problem40)
{
  using namespace roboptim;
  using namespace roboptim::schittkowski::problem40;
  ...
  OptimizationLogger<solver_t> logger
    (solver,
     "/tmp/roboptim-shared-tests/" SOLVER_NAME "/schittkowski/problem-40");
