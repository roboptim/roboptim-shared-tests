...
namespace roboptim
{
  namespace schittkowski
  {
	...
	static const double fx;
      ...
      const double ExpectedResult::f0 = -1000.;
      const double ExpectedResult::x[] = {20., 11., 15.};

      const double ExpectedResult::fx = -3300.;
	...
	const throw ()
      {
	grad.insert (0) = -1. * x[1] * x[2];
	grad.insert (1) = -1. * x[0] * x[2];
	grad.insert (2) = -1. * x[0] * x[1];
      }
...
BOOST_AUTO_TEST_CASE (schittkowski_problem26)
{
  ...
  problem.argumentBounds ()[0] = F<functionType_t>::makeInterval (0., 20.);
  problem.argumentBounds ()[1] = F<functionType_t>::makeInterval (0., 11.);
  problem.argumentBounds ()[2] = F<functionType_t>::makeInterval (0., 42.);
  ...
  OptimizationLogger<solver_t> logger
    (solver,
     "/tmp/roboptim-shared-tests/" SOLVER_NAME "/schittkowski/problem-37");
