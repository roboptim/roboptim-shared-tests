...
namespace roboptim
{
  namespace schittkowski
  {
	...
	static const double fx;
      ...
      const double ExpectedResult::x[] = {0., 1., 2., -1.};
      const double ExpectedResult::fx = -44.;
	...
	const throw ()
      {
	grad.insert (0) = 2. * x[0] - 5.;
	grad.insert (1) = 2. * x[1] - 5.;
	grad.insert (2) = 4. * x[2] - 21.;
	grad.insert (3) = 2. * x[3] + 7.;
      }
	...
	const throw ()
      {
	grad.insert (0) = -2. * x[0] + 1.;
	grad.insert (1) = -2. * 2. * x[1];
	grad.insert (2) = -2. * x[2];
	grad.insert (3) = -2. * 2. * x[3] + 1.;
      }
	...
	const throw ()
      {
	grad.insert (0) = -2. * 2. * std::pow (x[0], 2) - 2.;
	grad.insert (1) = -2. * x[1] + 1.;
	grad.insert (2) = -2. * x[2];
	grad.insert (3) = 1.;
      }
...
BOOST_AUTO_TEST_CASE (schittkowski_problem43)
{
  ...
  OptimizationLogger<solver_t> logger
    (solver,
     "/tmp/roboptim-shared-tests/" SOLVER_NAME "/schittkowski/problem-43");
