...
namespace roboptim
{
  namespace schittkowski
  {
	...
	static const double fx;
      ...
      const double ExpectedResult::f0 = 2.25;
      const double ExpectedResult::x[] = {4./3., 7./9., 4./9.};
      const double ExpectedResult::fx = 1./9.;
	...
	impl_gradient (gradient_ref grad, const_argument_ref& x, size_type)
      ...
      F<T>::F () throw ()
	: GenericDifferentiableFunction<T>
	  (3, 1, "9 - 8x₀ - 6x₁ - 4x₂ + 2x₀² + 2x₁² + x₂² + 2x₀x₁ + 2x₀x₂")
      {}
	...
	const throw ()
      {
	grad.insert (0) = 4 * x[0] + 2 * x[1] + 2 * x[2] - 8;
	grad.insert (1) = 2 * x[0] + 4 * x[1] - 6;
	grad.insert (2) = 2 * x[0] + 2 * x[2] - 4;
      }
	...
	const throw ()
      {
	grad[0] = 4 * x[0] + 2 * x[1] + 2 * x[2] - 8;
	grad[1] = 2 * x[0] + 4 * x[1] - 6;
	grad[2] = 2 * x[0] + 2 * x[2] - 4;
      }
    ...
    } // end of namespace problem35.
  } // end of namespace schittkowski.
} // end of namespace roboptim.

BOOST_FIXTURE_TEST_SUITE (schittkowski, TestSuiteConfiguration)

BOOST_AUTO_TEST_CASE (schittkowski_problem35)
{
  using namespace roboptim;
  using namespace roboptim::schittkowski::problem35;
  ...
  OptimizationLogger<solver_t> logger
    (solver,
     "/tmp/roboptim-shared-tests/" SOLVER_NAME "/schittkowski/problem-35");
