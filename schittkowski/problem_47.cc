...
namespace roboptim
{
  namespace schittkowski
  {
	...
	static const double fx;
      ...
      const double ExpectedResult::f0 = 20.7380774886;
	...
	grad.insert (1) = -2 * (x[0] - x[1]) + 3 * std::pow (x[1] - x[2], 2);
	grad.insert (2) = -3 * std::pow (x[1] - x[2], 2) + 4 * std::pow (x[2] - x[3], 3);
	grad.insert (3) = -4 * std::pow (x[2] - x[3], 3) + 4 * std::pow (x[3] - x[4], 3);
...
BOOST_AUTO_TEST_CASE (schittkowski_problem47)
{
  ...
  OptimizationLogger<solver_t> logger
    (solver,
     "/tmp/roboptim-shared-tests/" SOLVER_NAME "/schittkowski/problem-47");
