...
namespace roboptim
{
  namespace schittkowski
  {
	...
	static const double fx;
      ...
      const double ExpectedResult::f0 = 6256;
	...
	grad.insert (1) = 2. * (-x[0] + 2. * x[1] - x[2]);
	grad.insert (2) = 2. * (-x[1] + 2. * x[2] - x[3]);
	grad.insert (3) = 2. * (-x[2] + 2. * x[3] - x[4]);
	...
	const throw ()
      {
	grad[0] = 2. * (x[0] - x[1]);
	grad[1] = 2. * (-x[0] + 2. * x[1] - x[2]);
	grad[2] = 2. * (-x[1] + 2. * x[2] - x[3]);
	grad[3] = 2. * (-x[2] + 2. * x[3] - x[4]);
	grad[4] = 2. * (-x[3] + x[4]);
      }
	...
	const throw ()
      {
	result[0] = x[0] + 2 * x[1] + 3 * x[2];
	result[1] = x[1] + 2 * x[2] + 3 * x[3];
	result[2] = x[2] + 2 * x[3] + 3 * x[4];
      }

      template <>
      void
      G<EigenMatrixSparse>::impl_jacobian
      (jacobian_ref jac, const_argument_ref&) const throw ()
      {
	jac.insert (0,0) = 1;
	jac.insert (0,1) = 2;
	jac.insert (0,2) = 3;
    ...
    } // end of namespace problem50.
  } // end of namespace schittkowski.
} // end of namespace roboptim.

BOOST_FIXTURE_TEST_SUITE (schittkowski, TestSuiteConfiguration)

BOOST_AUTO_TEST_CASE (schittkowski_problem50)
{
  using namespace roboptim;
  using namespace roboptim::schittkowski::problem50;
  ...
  OptimizationLogger<solver_t> logger
    (solver,
     "/tmp/roboptim-shared-tests/" SOLVER_NAME "/schittkowski/problem-50");
