...
namespace roboptim
{
  namespace schittkowski
  {
	...
	const throw ()
      {
	grad[0] = 10. * x[0] - 2. * x[1] + 2. * x[2];
	grad[1] = -2. * x[0] + 26. * x[1] + 6. * x[2];
	grad[2] = 2. * x[0] + 6. * x[1] + 2. * x[2];
      }
	...
	const throw ()
      {
	result[0] = 6. * x[1] + 4. * x[2] - std::pow (x[0], 3) - 3.;
      }
    ...
    } // end of namespace problem32.
  } // end of namespace schittkowski.
} // end of namespace roboptim.

BOOST_FIXTURE_TEST_SUITE (schittkowski, TestSuiteConfiguration)

BOOST_AUTO_TEST_CASE (schittkowski_problem32)
{
  using namespace roboptim;
  using namespace roboptim::schittkowski::problem32;
  ...
  OptimizationLogger<solver_t> logger
    (solver,
     "/tmp/roboptim-shared-tests/" SOLVER_NAME "/schittkowski/problem-32");
