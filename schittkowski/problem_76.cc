...
namespace roboptim
{
  namespace schittkowski
  {
	...
	static const double fx;
      ...
      const double ExpectedResult::f0 = -1.25;
      const double ExpectedResult::x[] = {0.2727273, 2.090909, -0.26e-10,
                                          0.5454545};
      const double ExpectedResult::fx = -4.681818181;
	...
	const throw ()
      {
	grad.insert (0) = 2 * x[0] - x[2] - 1;
	grad.insert (1) = x[1] - 3;
	grad.insert (2) = 2 * x[2] - x[0] + 1;
	grad.insert (3) = x[3] + x[2] - 1;
      }
...
BOOST_AUTO_TEST_CASE (schittkowski_problem76)
{
  ...
  OptimizationLogger<solver_t> logger
    (solver,
     "/tmp/roboptim-shared-tests/" SOLVER_NAME "/schittkowski/problem-76");
