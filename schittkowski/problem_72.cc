...
namespace roboptim
{
  namespace schittkowski
  {
	...
	static const double fx;
      ...
      const double ExpectedResult::f0 = 5;
      const double ExpectedResult::x[] = {193.4071, 179.5475, 185.0186,
                                          168.7062};
      const double ExpectedResult::fx = 727.67937;
	...
	impl_jacobian (jacobian_ref jac, const_argument_ref x)
      ...
      G<T>::G ()
	: GenericDifferentiableFunction<T>
	  (4, 2, "0.0401 - 4/x₀ - 2.25/x₁ - 1/x₂ - 0.25/x₃, "
	   "0.010085 - 0.16/x₀ - 0.36/x₁ - 0.64/x₂ - 0.64/x₃")
      {}
	...
	const
      {
	result[0] = 0.0401 - 4. / x[0] - 2.25 / x[1] - 1. / x[2] - 0.25 / x[3];
	result[1] = 0.010085 - 0.16 / x[0] - 0.36 / x[1] - 0.64 / x[2] - 0.64 / x[3];
      }
      ...
      {
	jac.coeffRef (0,0) = 4.   / (x[0] * x[0]);
	jac.coeffRef (0,1) = 2.25 / (x[1] * x[1]);
	jac.coeffRef (0,2) = 1.   / (x[2] * x[2]);
	jac.coeffRef (0,3) = 0.25 / (x[3] * x[3]);

	jac.coeffRef (1,0) = 0.16 / (x[0] * x[0]);
	jac.coeffRef (1,1) = 0.36 / (x[1] * x[1]);
	jac.coeffRef (1,2) = 0.64 / (x[2] * x[2]);
	jac.coeffRef (1,3) = 0.64 / (x[3] * x[3]);
      }
      ...
      {
	jac (0,0) = 4.   / (x[0] * x[0]);
	jac (0,1) = 2.25 / (x[1] * x[1]);
	jac (0,2) = 1.   / (x[2] * x[2]);
	jac (0,3) = 0.25 / (x[3] * x[3]);

	jac (1,0) = 0.16 / (x[0] * x[0]);
	jac (1,1) = 0.36 / (x[1] * x[1]);
	jac (1,2) = 0.64 / (x[2] * x[2]);
	jac (1,3) = 0.64 / (x[3] * x[3]);
      }
    } // end of namespace problem72.
  } // end of namespace schittkowski.
} // end of namespace roboptim.

BOOST_FIXTURE_TEST_SUITE (schittkowski, TestSuiteConfiguration)

BOOST_AUTO_TEST_CASE (schittkowski_problem72)
{
  using namespace roboptim;
  using namespace roboptim::schittkowski::problem72;
