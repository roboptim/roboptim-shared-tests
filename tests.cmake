...
MACRO(BUILD_TEST FILE_NAME)
  ...
  IF(NOT(MSVC))
    SET_TARGET_PROPERTIES(${EXE_NAME}${PROGRAM_SUFFIX}
      ...
      "-DSOLVER_NAME='\"${SOLVER_NAME}\"' -DPLUGIN_PATH='\"${PLUGIN_PATH}\"' -DFUNCTION_TYPE=${FUNCTION_TYPE} -DTESTS_DATA_DIR='\"${CMAKE_CURRENT_SOURCE_DIR}\"' -DLOG_FILENAME='\"${EXE_NAME}${PROGRAM_SUFFIX}.log\"' -DLOG_DIR='\"${EXE_NAME}${PROGRAM_SUFFIX}_log\"' ")
  ELSE()
    SET_TARGET_PROPERTIES(${EXE_NAME}${PROGRAM_SUFFIX}
      PROPERTIES COMPILE_FLAGS
      "-DSOLVER_NAME#\\\"${SOLVER_NAME}\\\" -DPLUGIN_PATH#\\\"${PLUGIN_PATH}\\\" -DFUNCTION_TYPE#${FUNCTION_TYPE} -DTESTS_DATA_DIR#\\\"${CMAKE_CURRENT_SOURCE_DIR}\\\" -DLOG_FILENAME#\\\"${EXE_NAME}${PROGRAM_SUFFIX}.log\\\" -DLOG_DIR#\\\"${EXE_NAME}${PROGRAM_SUFFIX}_log\\\" ")
