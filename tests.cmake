...
MACRO(BUILD_TEST FILE_NAME)
  ...
  IF(WIN32)
    SET_PROPERTY(
      ...
      "PATH=${Boost_LIBRARY_DIR_RELEASE}${SEP}${CMAKE_INSTALL_PREFIX}/bin${SEP}${CMAKE_INSTALL_PREFIX}/lib/roboptim-core${SEP}$ENV{PATH}"
    )
  ENDIF()

  set(LTDL_LIBRARY_PATH "${CMAKE_BINARY_DIR}/src${SEP}$ENV{LTDL_LIBRARY_PATH}")
  IF(MSVC AND "${CMAKE_GENERATOR}" MATCHES "Visual Studio.*")
    FOREACH(conf ${CMAKE_CONFIGURATION_TYPES})
      set(LTDL_LIBRARY_PATH "${CMAKE_BINARY_DIR}/src/${conf}${SEP}${LTDL_LIBRARY_PATH}")
    ENDFOREACH()
  ENDIF()
  SET_PROPERTY(
    TEST ${EXE_NAME}${PROGRAM_SUFFIX} APPEND PROPERTY ENVIRONMENT
  ...
  IF(UNIX)
    IF(APPLE)
      SET_PROPERTY(
        TEST ${EXE_NAME}${PROGRAM_SUFFIX} APPEND PROPERTY ENVIRONMENT
        "DYLD_LIBRARY_PATH=${CMAKE_BINARY_DIR}/src${SEP}$ENV{DYLD_LIBRARY_PATH}")
    ELSE()
      SET_PROPERTY(
        TEST ${EXE_NAME}${PROGRAM_SUFFIX} APPEND PROPERTY ENVIRONMENT
        "LD_LIBRARY_PATH=${CMAKE_BINARY_DIR}/src${SEP}$ENV{LD_LIBRARY_PATH}")
    ENDIF()
  ENDIF()
ENDMACRO()
